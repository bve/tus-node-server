...
class PostHandler extends BaseHandler {
    ...
    send(req, res) {
        return this.store.create(req)
            .then((File) => {
                const url = this.store.relativeLocation ? `${req.baseUrl || ''}${this.store.path}/${File.id}` : `http://${req.headers.host}${req.baseUrl || ''}${this.store.path}/${File.id}`;

                this.emit(EVENT_ENDPOINT_CREATED, { url });
            
                let body = `{"success": true, "message": "", "fileId": ${File.id}}`;
                res.writeHead(201, { 'Content-Length': body.length, Location: url });
                res.write(body);
